...
export const PersonalProfileInformation = async (req, res) => {
    try {
...
export const LandInformation = async (req, res) => {
    try {
        ...
        const validCoordinates = coordinates.map((c) => ({
...
export const DocumentInformation = async (req, res) => {
    try {
        if (!req.user || !req.user.farmerId) {
            return res.status(401).json({ error: "Unauthorized" });
        }
...
export const CropInformation = async (req, res) => {
    try {
        if (!req.user || !req.user.farmerId) {
            return res.status(401).json({ error: "Unauthorized" });
        }
...
//# sourceMappingURL=farmer.profile.controller.js.map